...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      token,
      user: { id: result.insertId, name, email, role, mobile, location },
    });
  } catch (err) {
...
// Simulated: in production wire up SES/nodemailer. Returns reset token in response for demo.
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    // In production: send email via AWS SES with reset link
